...
def recognize(img_path, tesseract_path):  # Taken from GFG
    ...
    pytesseract.pytesseract.tesseract_cmd = tesseract_path  # '/usr/bin/tesseract'
...
text = recognize('employee.png', '/usr/bin/tesseract')
...
class TableNameException(Exception):
    def __init__(self):
        self.msg = "\nYour table name isn't in one word. Consider using underscores (_) instead of spaces and hyphens(-)\n"
